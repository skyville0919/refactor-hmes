...
<?php
    ...
    if(isset($_SESSION['admin_accounts'])) {
        $session = "SELECT * FROM admin_accounts WHERE Acc_ID=".$_SESSION['admin_accounts'];
        $sessionres = mysqli_query($db, $session);
        $rowses = mysqli_fetch_array($sessionres);
        if ($rowses['Role'] == "HOUSEKEEPING-ADMIN") {
            header("location:housekeeping/housekeeping-dashboard.php");
            exit;
        }
        else if  ($rowses['Role'] == "ACCOUNTING-ADMIN") {
            header("location:accounting/accounting.php");
            exit;
        }
        else if  ($rowses['Role'] == "ACCOUNT-ADMIN") {
            header("location:account/accounts-dashboard.php");
            exit;
        }
        else if  ($rowses['Role'] == "INVENTORY-ADMIN") {
            header("location:inventory/inventory-dashboard.php");
            exit;
        }
        else if  ($rowses['Role'] == "CMS-ADMIN") {
            header("location:cms/cms.php");
            exit;
        }
        else if  ($rowses['Role'] == "RESERVATION-ADMIN") {
            header("location:housekeeping/housekeeping-dashboard.php");
            exit;
        }
    }
